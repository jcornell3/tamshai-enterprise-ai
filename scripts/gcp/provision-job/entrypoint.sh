...
# -----------------------------------------------------------------------------
load_finance_data() {
    if [ "$ACTION" != "load-finance-data" ]; then
    ...
    if [ ! -f /app/sample-data/finance-data.sql ]; then
        echo "[WARN] Finance data file not found: /app/sample-data/finance-data.sql"
        return 0
    fi

    export PGPASSWORD="$DB_PASSWORD"

    # Create tamshai_finance database if it doesn't exist
    echo "[INFO] Ensuring tamshai_finance database exists..."
    psql -h localhost -p 5432 -U tamshai -d postgres -c \
        "SELECT 1 FROM pg_database WHERE datname = 'tamshai_finance'" | grep -q 1 || \
        psql -h localhost -p 5432 -U tamshai -d postgres -c "CREATE DATABASE tamshai_finance;" 2>/dev/null || true

    echo "[INFO] Loading Finance data from sample-data/finance-data.sql..."
    psql -h localhost -p 5432 -U tamshai -d tamshai_finance -f /app/sample-data/finance-data.sql

    # Verify
    INVOICE_COUNT=$(psql -h localhost -p 5432 -U tamshai -d tamshai_finance -t -c \
        "SELECT COUNT(*) FROM finance.invoices;" 2>/dev/null | tr -d ' \n' || echo "0")
    echo "[OK] Finance data loaded: $INVOICE_COUNT invoices"
}
