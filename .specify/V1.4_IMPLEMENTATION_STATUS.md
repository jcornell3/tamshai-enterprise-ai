...
| delete_opportunity | ⚠️ Partial | Confirmation, Role Filtering | MongoDB | Tested (Dec 9) - Field issues |
...
3. **delete_opportunity** (Dec 9, 2025)
   - ✅ Returns pending_confirmation for eligible deals
   - ✅ Confirmation stored in Redis with 5-minute TTL
   - ✅ Business rule validation (cannot delete won deals)
   - ⚠️ **customer_name field undefined** (needs aggregation from customers collection)
   - ⚠️ **status field undefined** (actual field is `stage` not `status`)
   - ✅ Confirmation flow structure operational
   - ✅ v1.4 patterns verified despite field mismatches

**Outstanding Issues** (Lesson 5 follow-up required):
- ❌ Field name mismatch: `stage` (actual) vs `status` (code expects)
  - Impact: Confirmation message shows "Status: undefined"
  - Fix: Change line 159 from `opportunity.status === 'won'` to `opportunity.stage === 'CLOSED_WON'`
- ❌ Field name mismatch: `customer_id` (actual) vs `customer_name` (code expects)
  - Impact: Confirmation message shows "Delete opportunity for undefined?"
  - Fix: Add aggregation pipeline to lookup customer name from customers collection
  - Example: `collection.aggregate([{$match: {...}}, {$lookup: {from: 'customers', ...}}])`
- ⚠️ Status filtering broken due to field mismatch
- ⚠️ Cannot verify "won" deal protection logic (using wrong field)
