...
**Keycloak Admin Passwords** (environment-specific):
- `DEV_KEYCLOAK_ADMIN_PASSWORD` - Dev environment (local Docker)
- `KEYCLOAK_VPS_ADMIN_PASSWORD` - Stage/VPS environment (GitHub Secret, auto-generated by Terraform)
- `KEYCLOAK_ADMIN_PASSWORD` - Prod/GCP environment (GCP Secret Manager)

**User Passwords**:
- `DEV_USER_PASSWORD` - Default password for HR-synced users in dev
- `STAGE_USER_PASSWORD` - Default password for HR-synced users in stage
- `PROD_USER_PASSWORD` - Default password for HR-synced users in prod

**Test User**:
- `TEST_PASSWORD` - Password for test-user.journey (same across all environments)
- `TEST_USER_TOTP_SECRET` - BASE32 TOTP secret for E2E tests
