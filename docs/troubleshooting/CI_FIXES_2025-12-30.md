...
**Problem**: `docker-compose.yml` validation failed due to missing environment variables in CI.
...
✅ **No breaking changes** - workflow validation now passes in CI
- Real VPS deployment still uses actual `.env` file on the server
- Only CI validation uses test environment variables

### Integration Testing
⚠️ **Breaking change** - realm import removed:
...
### Local Development
✅ **No changes** - local docker-compose still uses realm import:
...
|----------|--------|-------|
| CI (main) | 🟡 **In Progress** | Integration tests now using API-based realm setup |
| Security Review | ✅ **Passing** | No changes needed |
| Deploy to VPS | ✅ **Passing** | Validation fixed with env vars |
| Tamshai DevSecOps Pipeline | 🟡 **In Progress** | Terraform credentials still needed (non-blocking) |
...
**Current Status**: ✅ **Fixed** (awaiting CI verification)
...
**Impact**:
- ✅ **Keycloak realm setup**: FIXED (Terraform eliminates HTTP 400 errors)
- ✅ **JWT issuer mismatch**: FIXED (Gateway aligned to 127.0.0.1)
- ⏳ **Integration tests**: Expected 74/74 passing (awaiting verification)
...
- ❌ **Missing in production stage**: `package-lock.json` and devDependencies (jest, etc.)

When CI ran `docker compose run --rm mcp-gateway sh -c "npm ci && npm test"`, it executed in the production container which:
1. Doesn't have `package-lock.json` → npm ci fails
2. Doesn't have devDependencies → npm test would fail even if npm ci worked
...
**Impact**:
- ✅ npm ci now runs with access to package-lock.json
- ✅ Tests run with full devDependencies
...
1. ✅ **Keycloak realm setup script** - Implemented `setup-keycloak-realm.sh`
2. ✅ **CI integration** - Script runs automatically in GitHub Actions
3. ✅ **Comprehensive documentation** - See `docs/KEYCLOAK_23_DEEP_DIVE.md`
4. ✅ **Terraform deployment** - Replaced bash script, zero HTTP 400 errors
5. ✅ **Fix 401 test failures** - Removed non-existent "roles" scope (commit b1bdb9d)
6. ✅ **Add comprehensive CI debug step** - Verifies token acquisition before tests
7. ⏳ **Monitor CI run** - Verify all 74/74 integration tests pass
8. ⏭️ **Fix Terraform credentials** in deploy.yml (optional - only for deployments)
...
| `c54b653` | Add test env vars for docker-compose.yml validation |
...
**Investigation Path**:
1. ✅ Removed "roles" scope from tests → Still `resource_access: null`
2. ✅ Converted roles from realm to client roles → Still `resource_access: null`
3. ✅ Found the actual issue: `full_scope_allowed = false` in client config
...
- **Our use case**: Internal client with our own roles → `true` is appropriate
...
### 11. Research Team Findings - Dual Root Cause (2025-12-31)

**Analysis**: Research team identified TWO distinct critical issues

#### Issue 1: Logic Bug - Gateway Ignores Client Roles
**Root Cause**: `services/mcp-gateway/src/index.ts:217` only reads `realm_access.roles`, completely ignoring `resource_access["mcp-gateway"].roles` where Keycloak stores client roles.
...
// ... later (Line 259):
roles: realmRoles,  // ❌ Only realm roles, ignores client roles
```

**Impact**: Even if Keycloak sends client roles in `resource_access`, gateway code blindly ignores them, resulting in users with no permissions.

#### Issue 2: 401 Error - Issuer Mismatch in Performance/E2E Tests
**Root Cause**: Tests use `127.0.0.1:8180` (from jest.setup.js) but Performance/E2E test jobs start gateway with `localhost:8180`.

**Evidence**:
- Integration Tests job: ✅ Already had `KEYCLOAK_URL: http://127.0.0.1:8180`
- Performance Tests job: ❌ Had `KEYCLOAK_URL: http://localhost:8180`
- E2E Tests job: ❌ Had `KEYCLOAK_URL: http://localhost:8180`

**Impact**: JWT signature validation fails because `iss` claim (`127.0.0.1`) doesn't match gateway's expected issuer (`localhost`).

#### Fix 1: Support Client Roles in Gateway
```typescript
// services/mcp-gateway/src/index.ts:216-223
const realmRoles = payload.realm_access?.roles || [];
...
**Benefits**:
- Supports both realm roles (legacy) and client roles (best practice)
- Backward compatible (still works with realm roles)
- Deduplicates overlapping roles

#### Fix 2: Force Issuer Match in Performance/E2E Tests
```yaml
# .github/workflows/ci.yml (Performance/E2E jobs)
- name: Start MCP Gateway
  env:
    # CRITICAL: Use 127.0.0.1 to match test runner (jest.setup.js)
    KEYCLOAK_URL: http://127.0.0.1:8180  # Changed from localhost
```

**Files Changed**:
- `services/mcp-gateway/src/index.ts` (lines 216-239, 268)
- `.github/workflows/ci.yml` (lines 611, 680)
