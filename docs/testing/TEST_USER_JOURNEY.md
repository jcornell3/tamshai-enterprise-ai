...
| **Password** | `Test123!Journey` | Strong password for security |
...
test('test-user.journey can login and access app', async ({ page }) => {
  ...
  await page.fill('input[name="password"]', 'Test123!Journey');
...
1. **Password Strength**: Uses a strong password (`Test123!Journey`)
...
- `TEST_USER_PASSWORD`: `Test123!Journey`
