...
- **Base URL**: `https://www.tamshai.local:${PORT_CADDY_HTTPS}`
- **Keycloak URL**: `https://www.tamshai.local:${PORT_CADDY_HTTPS}/auth` (via Caddy) or `http://localhost:${PORT_KEYCLOAK}/auth` (direct)
- **Keycloak Admin**: `https://www.tamshai.local:${PORT_CADDY_HTTPS}/auth/admin` (admin/admin)
- **Client ID**: `tamshai-website` (employee), `customer-portal` (customer)
- **Customer Portal**: `https://customers.tamshai.local:${PORT_CADDY_HTTPS}`

> **Port variables** are defined in `infrastructure/docker/.env` (generated by Terraform). Default dev values: `PORT_CADDY_HTTPS=443`, `PORT_KEYCLOAK=8180`.
...
**Container names**: Docker Compose uses project name `tamshai-dev`, so containers are prefixed `tamshai-dev-` (e.g., `tamshai-dev-keycloak`, not `tamshai-keycloak`).
