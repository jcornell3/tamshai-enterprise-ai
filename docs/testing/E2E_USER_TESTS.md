...
# Load all credentials from GitHub Secrets (RECOMMENDED)
...
**⚠️ IMPORTANT: Do NOT rely on `.env` files for secrets.**

The `infrastructure/docker/.env` file is Terraform-generated and may contain stale or environment-mismatched values. Always use the GitHub Secrets export workflow:

```bash
# Export secrets from GitHub (requires gh CLI authenticated)
# Run from repo root, then trigger workflow in GitHub Actions UI
# Workflow: "Export Test Secrets" → select "e2e" type → download artifact

# Load exported secrets as environment variables
eval $(../../scripts/secrets/read-github-secrets.sh --e2e --env)
```

**Why not .env files?**
- `.env` files are generated by Terraform and may be stale
- Secrets may not match the target test environment (dev vs stage vs prod)
- GitHub Secrets are the single source of truth for test credentials
- The export workflow ensures you have current, valid secrets
...
| `MCP_INTEGRATION_RUNNER_SECRET` | For API/integration tests | GitHub Secret: `MCP_INTEGRATION_RUNNER_SECRET` | Service account secret for token exchange |
| `DEV_USER_PASSWORD` | For API tests | GitHub Secret (via e2e export) | Password for corporate users (eve.thompson, etc.) |
| `CUSTOMER_USER_PASSWORD` | For customer tests | GitHub Secret (via e2e export) | Password for customer realm users |
...
**Note on MCP_INTEGRATION_RUNNER_SECRET**: This secret is used by both E2E API tests and integration tests for token exchange authentication. E2E browser tests authenticate as real users (test-user.journey) via Keycloak login forms. API tests use token exchange to impersonate users when calling MCP server APIs directly. See [Integration Test README](../../tests/integration/README.md) for details.
